...
void PdfMemDocument::DeletePages( int inFirstPage, int inNumPages )
{
    ...
    {
		/*PdfPage *page = GetPage(inFirstPage);
		PdfObject *o = page->GetObject();
		TPdfReferenceList lst;
		GetObjects().GetObjectDependencies(o,&lst);
		for( int j = 0; j < lst.size(); j++)
		{
			PdfReference ref = lst.at(j);
			PdfObject *obj = GetObjects().GetObject(ref);
			PdfMemStream* pStream = (PdfMemStream *)o->GetStream();
			pStream->Empty();
		}*/
